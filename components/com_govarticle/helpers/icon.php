...
/**
 * GovArticle Component HTML Helper
 *
 * @since  1.5
 */
abstract class JHtmlIcon
{
	...
	public static function create($category, $params, $attribs = array(), $legacy = false)
	{
		...
		$url = 'index.php?option=com_govarticle&task=article.add&return=' . base64_encode($uri) . '&a_id=0&catid=' . $category->id;
		...
		$output = '<span class="hasTooltip" title="' . JHtml::tooltipText('COM_GOVARTICLE_CREATE_ARTICLE') . '">' . $button . '</span>';
	...
	public static function email($article, $params, $attribs = array(), $legacy = false)
	{
		...
		$link     = $base . JRoute::_(GovArticleHelperRoute::getArticleRoute($article->slug, $article->catid, $article->language), false);
	...
	public static function edit($article, $params, $attribs = array(), $legacy = false)
	{
			...
			&& $article->checked_out != $user->get('id'))
		{
			...
			$tooltip      = JText::_('JLIB_HTML_CHECKED_OUT') . ' :: ' . JText::sprintf('COM_GOVARTICLE_CHECKED_OUT_BY', $checkoutUser->name)
				. ' <br /> ' . $date;
		...
		$url = 'index.php?option=com_govarticle&task=article.edit&a_id=' . $article->id . '&return=' . base64_encode($uri);
		...
		$overlib .= $date;
		$overlib .= '&lt;br /&gt;';
		$overlib .= JText::sprintf('COM_GOVARTICLE_WRITTEN_BY', htmlspecialchars($author, ENT_COMPAT, 'UTF-8'));
		...
		else
		{
			...
			$text = '<span class="hasTooltip icon-' . $icon . ' tip" title="' . JHtml::tooltipText(JText::_('COM_GOVARTICLE_EDIT_ITEM'), $overlib, 0, 0)
				. '"></span>'
	...
	public static function print_popup($article, $params, $attribs = array(), $legacy = false)
	{
		...
		$url  = GovArticleHelperRoute::getArticleRoute($article->slug, $article->catid, $article->language);
