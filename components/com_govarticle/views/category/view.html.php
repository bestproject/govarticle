 ...
 * @since  1.5
 */
class GovArticleViewCategory extends JViewCategory
{
	/**
	...
	public function display($tpl = null)
	{
		...
		if (($params->get('layout_type') == 'blog') || ($this->getLayout() == 'blog'))
		{
			...
			if ($order == 0 && $this->columns > 1)
			{
				...
				$this->intro_items = GovArticleHelperQuery::orderDownColumns($this->intro_items, $this->columns);
			}
		}
	...
	protected function prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] != 'com_govarticle' || $menu->query['view'] == 'article' || $id != $this->category->id))
		{
			$path = array(array('title' => $this->category->title, 'link' => ''));
			$category = $this->category->getParent();

			while (($menu->query['option'] != 'com_govarticle' || $menu->query['view'] == 'article' || $id != $category->id) && $category->id > 1)
			{
