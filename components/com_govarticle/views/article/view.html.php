...
/**
 * HTML Article View class for the GovArticle component
 *
 * @since  1.5
 */
class GovArticleViewArticle extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$item->readmore_link = JRoute::_(GovArticleHelperRoute::getArticleRoute($item->slug, $item->catid, $item->language));
		...
		$item->tags->getItemTags('com_govarticle.article', $this->item->id);
	...
	protected function _prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] != 'com_govarticle' || $menu->query['view'] != 'article' || $id != $this->item->id))
		{
			...
			$category = JCategories::getInstance('GovArticle')->get($this->item->catid);

			while ($category && ($menu->query['option'] != 'com_govarticle' || $menu->query['view'] == 'article' || $id != $category->id) && $category->id > 1)
			{
				$path[]   = array('title' => $category->title, 'link' => GovArticleHelperRoute::getCategoryRoute($category->id));
