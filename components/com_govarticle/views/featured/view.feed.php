 ...
 * @since  1.5
 */
class GovArticleViewFeatured extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$doc->link = JRoute::_('index.php?option=com_govarticle&view=featured');
		...
		foreach ($rows as $row)
		{
			...
			// Url link to article
			$link = JRoute::_(GovArticleHelperRoute::getArticleRoute($row->slug, $row->catid, $row->language));
			...
			{
				$description .= '<p class="feed-readmore"><a target="_blank" href ="' . $item->link . '">' . JText::_('COM_GOVARTICLE_FEED_READMORE') . '</a></p>';
			}
