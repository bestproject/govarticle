...
/**
 * GovArticleNavigation plugin class.
 *
 * @since  1.5
 */
class PlgContentGovArticleNavigation extends JPlugin
{
	/**
	...
	public function onContentBeforeDisplay($context, &$row, &$params, $page = 0)
	{
		...
		if (($context == 'com_content.article' OR $option == 'com_govarticle') && ($view == 'article') && $params->get('show_item_navigation'))
		{
			$db       = JFactory::getDbo();
			...
			**/
			...
			if ($row->prev)
			{
				...
				$row->prev = JRoute::_(GovArticleHelperRoute::getArticleRoute($row->prev->slug, $row->prev->catid, $row->prev->language));
			}
			else
			{
				$row->prev_label = '';
			...
			if ($row->next)
			{
				...
				$row->next = JRoute::_(GovArticleHelperRoute::getArticleRoute($row->next->slug, $row->next->catid, $row->next->language));
			}
			else
			{
				$row->next_label = '';
			...
			if ($row->prev || $row->next)
			{
				...
				$path = JPluginHelper::getLayoutPath('content', 'govarticlenavigation');
