 ...
 * 
 * Based on plg_search_content from Joomla!
 */
 ...
 * @since  1.6
 */
class PlgSearchGovArticle extends JPlugin
{
	...
	{
		static $areas = array(
			'govarticle' => 'PLG_SEARCH_GOVARTICLE_AREA_GOVARTICLE'
		);
	...
	public function onContentSearch($text, $phrase = '', $ordering = '', $areas = null)
	{
		...
		if ($sContent && $limit > 0)
		{
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = GovArticleHelperRoute::getArticleRoute($item->slug, $item->catid, $item->language);
				}
			}
		...
		if ($sArchived && $limit > 0)
		{
			...
			$item = $app->getMenu()->getItems('link', 'index.php?option=com_govarticle&view=archive', true);
			...
			if (isset($list3))
			{
				foreach ($list3 as $key => $item)
				{
					...
					$list3[$key]->href = JRoute::_('index.php?option=com_govarticle&view=archive&year=' . $created_year . '&month=' . $created_month . $itemid);
				}
			}
