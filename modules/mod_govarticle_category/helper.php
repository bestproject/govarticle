...
$com_path = JPATH_SITE . '/components/com_govarticle/';
require_once $com_path . 'helpers/route.php';

JModelLegacy::addIncludePath($com_path . '/models', 'GovArticleModel');

/**
 * Helper for mod_govarticle_category
 */
abstract class ModGovArticleCategoryHelper {
	...
	public static function getList(&$params) {
		...
		$articles = JModelLegacy::getInstance('Articles', 'GovArticleModel', array('ignore_request' => true));
		...
		switch ($mode) {
			case 'dynamic' :
				...
				if ($option === 'com_govarticle') {
						...
						case 'article' :
							if ($params->get('show_on_article_page', 1)) {
								...
								if (!$catid) {
									...
									$article = JModelLegacy::getInstance('Article', 'GovArticleModel', array('ignore_request' => true));
					...
					// Return right away if not on a com_govarticle page
					return;
				}
		...
		if ($catids) {
			if ($params->get('show_child_category_articles', 0) && (int) $params->get('levels', 0) > 0) {
				...
				$categories = JModelLegacy::getInstance('Categories', 'GovArticleModel', array('ignore_request' => true));
		...
		if ($option === 'com_govarticle' && $view === 'article') {
		...
		foreach ($items as &$item) {
			...
			if ($item->catid) {
				$item->displayCategoryLink = JRoute::_(GovArticleHelperRoute::getCategoryRoute($item->catid));
			...
			if ($show_introtext) {
				$item->introtext = JHtml::_('content.prepare', $item->introtext, '', 'mod_govarticle_category.content');
